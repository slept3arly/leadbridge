...
type Activity = {
  id: string;
  type: string;
  metadata: { noteId?: string; oldContent?: string } | null;
};
...
export function NotesPanel({ leadId, currentUserId, isAdmin }: { leadId: string; currentUserId: string; isAdmin: boolean }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      for (const act of actRes.data as Activity[]) {
        if (act.type === "NOTE_EDITED" && act.metadata?.noteId && act.metadata?.oldContent) {
          history[act.metadata.noteId] = act.metadata.oldContent;
        }
      }
  ...
  async function updateNote(id: string) {
    ...
    setSaving(true);
    try {
      const res = await axios.patch(`/api/notes/${id}`, { content: editContent });
      setNotes((prev) => prev.map((n) => n.id === id ? { ...n, content: editContent, editedAt: new Date().toISOString() } : n));
      setEditingId(null);
      setEditContent("");
  ...
  return (
    <div className="space-y-4">
      ...
      {notes.length === 0 ? (
        ...
        <ExpandableSection initialVisible={5} itemCount={notes.length}>
          {notes.map((note) => {
